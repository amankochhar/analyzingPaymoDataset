...
The python file (antifraud.py) is in "src" folder. It takes the input files (batch_payment.txt, stream_payment.txt) from "paymo_input" folder 
and saves the results (output1.txt, output2.txt, output3.txt) in "paymo_output" folder.
...
EXTRA FEATURES:
Friend list: The user can see his friend list and the user he has in his trust list. The number of friends and there name/id can be seen by calling the id of the concerned user.

Remove Friend: If for some reason the user wants to remove a person from his list of trusted users he can do so. To do so call the function removeFriend() with the concerned user id and the id of the user to remove. Once called the person will be removed and any future transactions with this person will be marked "unverified". 
